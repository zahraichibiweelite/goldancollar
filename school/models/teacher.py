...
class SchoolTeacher(models.Model):
    ...
    employee_id = fields.Many2one('hr.employee', 'Employé ID',
                                  ondelete="cascade",
                                  delegate=True, required=True)
    standard_id = fields.Many2one('school.standard',
                                  "Responsabilité de la classe académique",
                                  help="Norme pour laquelle l'enseignant \
                                   responsable de.")
    stand_id = fields.Many2one('standard.standard', "Cours",
    ...
    category_ids = fields.Many2many('hr.employee.category',
                                    ...
                                    'categ_id', 'Tags')
    department_id = fields.Many2one('hr.department', 'Départment')
    is_parent = fields.Boolean('Est parent')
    stu_parent_id = fields.Many2one('school.parent', 'Parent apparenté')
    student_id = fields.Many2many('student.student',
                                  'students_teachers_parent_rel',
                                  'teacher_id', 'student_id',
                                  'Enfants')
    phone_numbers = fields.Char("Numéro de téléphone")
